...
# Default Seed User (can be configured via env vars)
SEED_USER_ID = uuid.UUID(os.getenv("SEED_USER_ID", "00000000-0000-0000-0000-000000000001"))
...
async def main():
    ...
    # Truncate tables (except users) and reset database sequence IDs
    async with engine.begin() as conn:
        print("Truncating existing academic/settings/todos tables and resetting sequences...")
        await conn.execute(
            text(
    ...
    async with async_session() as session:
        # Check if users table is empty; if so, create the default seed user
        users_res = await session.execute(text("SELECT id, email FROM users;"))
        users = users_res.all()
        
        if not users:
            print("No users found in database. Seeding default test user...")
            await session.execute(
                ...
                {"user_id": SEED_USER_ID, "email": SEED_USER_EMAIL}
            )
        ...
        for user_row in users:
            ...
            print(f"\n==========================================")
            print(f"SEEDING DATA FOR USER: {user_email} ({user_id})")
            print(f"==========================================")
            
            # Set the context-local user ID so model defaults resolve correctly
            request_user_id.set(user_id)
            
            # 1. Ensure app_settings exists for this user
            settings_res = await session.execute(
                text("SELECT 1 FROM app_settings WHERE user_id = :user_id;"),
                {"user_id": user_id}
            )
            if not settings_res.scalar():
                print(f"Creating default app settings for user {user_email}...")
                await session.execute(
                    text(
                        "INSERT INTO app_settings (user_id, theme_mode, finance_enabled, "
                        "morning_digest_enabled, night_digest_enabled, attendance_prompt_enabled, created_at, updated_at) "
                        "VALUES (:user_id, 'SYSTEM', false, true, true, true, NOW(), NOW());"
                    ),
                    {"user_id": user_id}
                )
                await session.flush()

            # Instantiate repositories for this user
            semester_repo = SemesterRepository(session, user_id=user_id)
            attendance_repo = AttendanceSettingsRepository(session)
            subject_repo = SubjectRepository(session)
            notes_subject_repo = NotesSubjectRepository(session, user_id=user_id)
            ...
            todo_repo = TodoRepository(session, user_id=user_id)
            ...
            # 5. Create Lecture Templates (timetable)
            ...
            # 7. Set Active Semester in app_settings
            print("Setting Semester 1 as active in app_settings...")
            await session.execute(
                text("UPDATE app_settings SET active_semester_id = :sem_id WHERE user_id = :user_id;"),
                {"sem_id": semester.semester_id, "user_id": user_id}
            )
            await session.commit()
            print(f"Seeding completed successfully for user {user_email}!")
